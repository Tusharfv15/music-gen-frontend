...
export const generateSong = inngest.createFunction(
  {
    ...
    onFailure: async ({ event, error }) => {
      await db.song.update({
        where: {
          id: (event?.data as { event?: { data?: { songId?: string } } })?.event?.data?.songId,
        },
        data: {
          status: "failed",
        },
  ...
  async ({ event, step }) => {
    ...
    const { userId, credits, endpoint, body } = await step.run(
      ...
      async () => {
        ...
        // Custom mode : Both lyrcis and prompt
    ...
    if (credits > 0) {
      ...
      }); // ← Added missing closing bracket and parenthesis for step.run callback
    ...
    } // ← Added missing closing bracket for the main if block
  },
);
